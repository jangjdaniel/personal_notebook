# 📖 Daniel's Meta-Notebook for Biostatistics 📖

This is my meta-notebook, which contains information on current projects I am working on, any big picture items from aforementioned projects (such as simulations, proofs, and mistakes), and some miscellaneous documents (such as coding tutorials, teaching slides, and templates). Week-to-week items (such as meeting slides,  granular information on a given project, and manuscripts) lives in their individual GitHub repositories. This is subject to change as I adapt to this system.

I will also add all relevant posters and presentations of my research (outside of the classroom) as well as my CV. 

---

## 🗺️ Project Directory & Code Ecosystem

| Project Name | Repository Link | Statistical Methodology | Status |
| :--- | :--- | :--- | :--- |
| **Rotation #1** | [`repo-link`](#) | Nonmonotone Missing Data | 🟡 On Hold |
| **QSURE NMB** | [`repo-link`](#) | Noncollapsible GLMs and Measurement Error | 🟡 On Hold |
| **OARS** | [`repo-link`](#) | Weighted GEEs and Penalized Regression | 🟡 On Hold |

---

## 🛠️ Environment & Toolstack
* **Language:** R (v4.4+)
* **Primary Libraries:** `tidyverse`, `ggplot2`, `gt`
* **Render Pipeline:** Compiled locally

---

## 📥 Workflow & Contribution Rules
To maintain reproducibility and prevent merge conflicts, this repository strictly adheres to the following layout: (TBD)

---

## 📬 Contact
* **Researcher:** Daniel Jang (daniel.jang@pennmedicine.upenn.edu)
* **Institution:** University of Pennsylvania / Department of Biotatistics, Epidemiology and Informatics

